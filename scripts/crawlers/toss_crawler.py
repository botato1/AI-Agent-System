# scripts/crawlers/toss_crawler.py
import os
import sys
import json
import time
...
def get_article_links(page) -> list[str]:
    ...
    for page_num in range(1, MAX_PAGES + 1):
        ...
        page.goto(
            f"{TOSS_BLOG_URL}/category/engineering?page={page_num}",
            timeout=30000
        )
        ...
        links = page.eval_on_selector_all(
            "a",
            "els => els.map(el => el.href).filter(h => h.includes('/article/'))"
        )
...
def extract_date(page) -> str | None:
    ...
    try:
        time_date = page.get_attribute("time", "datetime")
        if time_date:
            return time_date
    except Exception:
        pass
...
def crawl_article(page, url: str) -> dict | None:
    try:
        ...
        title = page.title().replace(" | 토스 기술 블로그", "").strip()
...
def run():
    ...
    results = []
    existing_urls = set()
    output_path = os.path.join(OUTPUT_DIR, "toss_blog.json")
    ...
    def save():
        # 원자적 쓰기 — 임시파일에 쓴 뒤 교체 (중간 끊김 시 기존 파일 보존)
